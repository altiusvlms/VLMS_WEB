...
@Component({
  selector: 'vlms-enrol-list',
  templateUrl: './enrol-list.component.html',
  styleUrls: ['./enrol-list.component.scss']
})
export class EnrolListComponent implements OnInit {
...
export class CreateEnroll {
  ...
  uploadImages(event: any,index:any){
   ...
   saveCustomerEnrolment(){
     ...
     console.log(this.editDataEnrol)
    if (this.editDataEnrol) {
      ...
      ).pipe(untilDestroyed(this)).subscribe((updated: any) => {
        this.dialogRef.close(updated);
        this.enrollid = updated.resourceId;
